...
if __name__ == "__main__":
  ...
  flags.DEFINE_string("feature_sizes", "1024,128",
                      "特征向量的长度，1024对应rgb特征，128对应audio特征")
  ...
  flags.DEFINE_bool("start_new_model", True,
                    "是否生成新的模型，如果为True，将会直接删除模型存放路径中的所有模型，然后重新生成"
                    "如果是False，那么将根据路径中的checkpoint文件自动接着训练")
...
class Trainer(object):
  ...
  def __init__(self, cluster, task, train_dir, model, reader, model_exporter,
               log_device_placement=False, max_steps=None,
...
#                           task_as_string(self.task))
  ...
  def run(self, start_new_model=False):
    ...
    gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=FLAGS.gpu_ratio, allow_growth=False)
